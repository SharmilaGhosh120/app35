...
with st.container():
    ...
    try:
        ...
        if response.status_code == 200:
            st.image(logo_url, width=100, use_column_width=True, caption="Ky’ra Logo", output_format="JPEG")
        else:
...
if not st.session_state.authenticated:
    ...
    if st.button("Update Details"):
        if name_input:
            save_user(st.session_state.email, name_input, password_input, st.session_state.role)
    ...
    if st.session_state.role == "admin":
        ...
        if st.button("Register User"):
            if not new_email or not new_name or not new_password:
                ...
                save_user(new_email, new_name, new_password, new_role)
    ...
    def show_admin_dashboard(name):
        ...
        conn.close()
        if not users.empty:
            selected_student = st.selectbox("Select Student", users['email'].tolist(), 
                                            format_func=lambda x: f"{x} ({users[users['email'] == x]['name'].iloc[0]})")
            col1, col2 = st.columns(2)
            with col1:
                if st.button("Export Student CSV"):
                    csv_data = export_query_logs_to_csv(student_email=selected_student)
                    if csv_data:
                        st.download_button(
                        ...
                        st.error(f"No query logs for {selected_student}.")
            with col2:
                if st.button("Export Student PDF"):
                    pdf_data = export_query_logs_to_pdf(student_email=selected_student)
                    if pdf_data:
                        st.download_button(
            ...
            for project_title in query_logs['project_title'].unique():
                with st.expander(f"Query Logs for Project: {project_title}"):
                    ...
                    for _, row in project_logs.iterrows():
                        rating = row['feedback_rating'] if pd.notna(row['feedback_rating']) else "Not rated"
                        st.markdown(
                            f"<div class='history-entry'><strong>{row['name']} ({row['email']}) asked:</strong> {row['question']} <i>(at {format_timestamp(row['timestamp'])})</i><br><strong>Ky’ra replied:</strong> {row['response']}<br><strong>Rating:</strong> {rating}</div>",
                            unsafe_allow_html=True
                        )
                        st.markdown("---")
        else:
    ...
    def show_student_dashboard(name):
        ...
        if st.button("Submit Project"):
            if project_title:
                try:
                    save_project(st.session_state.email, project_title)
        ...
        if not query_text:
            ...
            try:
                ...
                with st.expander("🧠 Ky’ra’s Response", expanded=True):
                    st.markdown(
                        f"<div class='history-entry'><strong>Ky’ra’s Response:</strong> <br>{response}</br></div>",
                        unsafe_allow_html=True
                    )
    ...
    if not user_df.empty:
        ...
        for _, row in paginated_df.iterrows():
            ...
            with st.expander(f"Question at {format_timestamp(row['timestamp'])}"):
                st.markdown(
                    f"<div class='history-entry'><strong>You asked:</strong> {row['question']} <i>(at {format_timestamp(row['timestamp'])})</i><br><strong>Ky’ra replied:</strong> {response_text}<br><strong>Rating:</strong> {rating}</div>",
                    unsafe_allow_html=True
                )
                st.markdown("---")
        st.markdown('</div>', unsafe_allow_html=True)
